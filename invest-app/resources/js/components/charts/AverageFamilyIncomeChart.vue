  ...
  async mounted() {
    try {
      ...
      if (apiData && apiData.datasets && Array.isArray(apiData.datasets)) {
        const averageFamilyIncomeDataset = apiData.datasets.find(
          (dataset) => dataset.label === 'Average Family Income'
        )
        ...
        if (averageFamilyIncomeDataset) {
          ...
          this.chartData = {
            ...
            datasets: [averageFamilyIncomeDataset],
          }
        } else {
          console.error('Average Family Income dataset not found in API response.')
          this.chartData = null
        }
      } else {
